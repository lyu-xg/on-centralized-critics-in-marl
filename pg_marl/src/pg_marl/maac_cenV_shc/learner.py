...
class Learner(object):
    ...
    def train(self, epi_done, eps):
        ...
        state, obs, action, reward, n_state, n_obs, terminate, discnt, exp_v = zip(*cen_batch)
        ...
        n_obs = torch.cat(n_obs).view(batch_size, trace_len, -1)
        ...
        for _ in range(self.c_train_iteration):
            if not self.MC_baseline:
                ...
                if self.c_target:
                    Gt = self._get_bootstrap_return(reward, n_state, n_obs, terminate, epi_len, self.joint_critic_tgt_net)
                else:
                    Gt = self._get_bootstrap_return(reward, n_state, n_obs, terminate, epi_len, self.joint_critic_net)
            else:
                if self.c_target:
                    Gt = self._get_discounted_return(reward, n_state, n_obs, terminate, epi_len, self.joint_critic_tgt_net)
                else:
                    # G_t = r_t + gamma^1*r_t+1 + ....+ gamma^{T-t}*r_T with last step bootstrap
                    Gt = self._get_discounted_return(reward, n_state, n_obs, terminate, epi_len, self.joint_critic_net)

            TD = Gt - self.joint_critic_net(state, n_obs)[0]
        ...
        if not self.MC_baseline:
            ...
            Gt = self._get_discounted_return(reward, n_state, n_obs,  terminate, epi_len, self.joint_critic_net)
        V_value = self.joint_critic_net(state, n_obs)[0].detach()
    ...
    def _create_joint_critic(self):
        state_input_dim, obs_input_dim = self._get_input_shape()
        self.joint_critic_net = Critic(state_input_dim, obs_input_dim, 1, self.c_mlp_layer_size)
        self.joint_critic_tgt_net = Critic(state_input_dim, obs_input_dim, 1, self.c_mlp_layer_size)
        self.joint_critic_tgt_net.load_state_dict(self.joint_critic_net.state_dict())

    def _get_input_shape(self):
        return self.env.get_env_info()['state_shape'], sum(self.env.obs_size)
    ...
    def _get_discounted_return(self, reward, n_state, n_obs, terminate, epi_len, critic_net):
        ...
        for epi_idx, epi_r in enumerate(Gt):
            ...
            if not terminate[epi_idx][end_step_idx]:
                ...
                epi_r[end_step_idx] += self.gamma * critic_net(n_state[epi_idx].unsqueeze(0),
                                                               n_obs[epi_idx].unsqueeze(0))[0].detach()[0][end_step_idx]
            for idx in range(end_step_idx-1, -1, -1):
                epi_r[idx] = epi_r[idx] + self.gamma * epi_r[idx+1]
        return Gt

    def _get_bootstrap_return(self, reward, n_state, n_obs, terminate, epi_len, critic_net):
        if self.n_step_bootstrap:
            # implement n-step bootstrap
            bootstrap = critic_net(n_state, n_obs)[0].detach()
            Gt = copy.deepcopy(reward)
            for epi_idx, epi_r in enumerate(Gt):
                end_step_idx = epi_len[epi_idx]-1
                if not terminate[epi_idx][end_step_idx]:
            ...
            Gt = reward + self.gamma * critic_net(n_state, n_obs)[0].detach() * (-terminate + 1)
        return Gt
