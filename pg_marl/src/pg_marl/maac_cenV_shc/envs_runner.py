    ...
    try:
        while True:
            ...
            if cmd == 'step':
                ...
                child.send((last_state, last_obs, action, reward, state, obs, terminate, gamma**step))
...
class EnvsRunner(object):
    ...
    def _step(self, eps):
        ...
        for idx, parent in enumerate(self.parents):
            # env_return is (state, last_obs, a, r, obs, t)
            env_return = parent.recv()
            env_return = self._exp_to_tensor(idx, env_return)
            # env_return become (last_state, last_obs, a, r, state, obs, t, discnt, exp_valid)
    ...
    def _exp_to_tensor(self, env_idx, exp):
        # exp (last_state, last_obs, a, r, state, obs, t, discnt)
        ...
        disct = [torch.tensor(exp[7]).float().view(1,-1)] * self.n_agent
        ...
        return (last_state, last_obs, a, r, state, obs, t, disct, exp_v)
