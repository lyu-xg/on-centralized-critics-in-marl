...
class Learner(object):
    ...
    def __init__(self, env, controller, memory, obs_last_action=False, gamma=0.95, a_lr=1e-2, c_lr=1e-2, 
                 init_etrpy_w=0.0, end_etrpy_w=0.0, etrpy_w_l_d_epi=10000, 
    ...
    def train(self, epi_done, eps):
        ...
        TD = Gt - self.joint_critic_net(state)[0]
        self.joint_critic_loss = torch.mean(exp_v * TD * TD)
        ...
        if not self.MC_baseline:
            ...
            Gt = self._get_discounted_return(reward, n_state, terminate, epi_len, self.joint_critic_net)
        V_value = self.joint_critic_net(state)[0].detach()
        ...
        for agent, batch in zip(self.controller.agents, dec_batch):
            ...
            agent.actor_loss = -1 * torch.mean(actor_loss)
    ...
    def _cat_joint_exps(self, joint_exps):
        ...
        exp = []
        for o, a, r, o_n, t, discnt, exp_v in chain(*joint_exps):
            exp.append([torch.cat(o, dim=1).view(1,-1), 
    ...
    def _get_discounted_return(self, reward, n_state, terminate, epi_len, critic_net):
        ...
        for epi_idx, epi_r in enumerate(Gt):
            ...
            if not terminate[epi_idx][end_step_idx]:
                ...
                epi_r[end_step_idx] += self.gamma * critic_net(n_state[epi_idx].unsqueeze(0))[0].detach()[0][end_step_idx]
    ...
    def _get_bootstrap_return(self, reward, n_state, terminate, epi_len, critic_net):
        if self.n_step_bootstrap:
            ...
            bootstrap = critic_net(n_state)[0].detach()
            Gt = copy.deepcopy(reward)
            for epi_idx, epi_r in enumerate(Gt):
                end_step_idx = epi_len[epi_idx]-1
                if not terminate[epi_idx][end_step_idx]:
            ...
            Gt = reward + self.gamma * critic_net(n_state)[0].detach() * (-terminate + 1)
        return Gt
