...
from pg_marl.maac_cenV_rnn.config import ex
from pg_marl.maac_cenV_rnn.envs_runner import EnvsRunner
from pg_marl.maac_cenV_rnn.memory import Memory_epi
from pg_marl.maac_cenV_rnn.learner import Learner
from pg_marl.maac_cenV_rnn.controller import MAC
from pg_marl.maac_cenV_rnn.utils import Linear_Decay

@ex.main
def main(env_name, n_envs, total_epies, max_epi_steps, gamma, a_lr, c_lr, 
         eps_start, eps_end, eps_decay_epis, init_etrpy_w, end_etrpy_w, etrpy_w_l_d_epi, 
         ...
         a_mlp_layer_size, c_mlp_layer_size, a_rnn_layer_size, c_rnn_layer_size,
         discnt_a_loss, eval_policy, eval_freq, eval_num_epi, obs_last_action, prey_accel, prey_max_v, 
         obs_r, obs_resolution, flick_p, enable_boundary, benchmark, discrete_mul, 
         grid_dim, target_rand_move, random_init, small_box_only, terminal_reward_only, big_box_reward, small_box_reward, n_agent,
    ...
    for idx in range(n_run):
        ...
        env_params = {'max_epi_steps': max_epi_steps,
       ...
       # create env
        env = make_env(env_name, discrete_action_input=True, **env_params)
        env.seed(seed)

        # create buffer
        memory = Memory_epi(env.obs_size, env.n_action, obs_last_action, size=train_freq)
        # cretate controller
        ...
        # create learner
        learner = Learner(env, controller, memory, obs_last_action, **hyper_params, **critic_params)
