...
class Scenario(BaseScenario):
    ...
    def observation(self, agent, world):
        ...
        return np.around(np.concatenate([agent.state.p_vel] + entity_pos + [goal_color[1]] + comm), decimals=agent.obs_resolution)

    def state(self, world):

        # goal_color
        goal_color = [agent.goal_b.color for agent in world.agents]
        ...
        return np.around(np.concatenate(goal_color + agent_pos + agent_vel + entity_pos), decimals=world.agents[0].obs_resolution)
