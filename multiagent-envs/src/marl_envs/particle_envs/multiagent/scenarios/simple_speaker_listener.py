...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        # set the goal color to listener's color
    ...
    def observation(self, agent, world):
        ...
        # listener
        if agent.silent:
            return np.around(np.concatenate([agent.state.p_vel] + entity_pos + comm), decimals=agent.obs_resolution)
    
    def state(self, world):
        # goal_color
        goal_color = np.zeros(world.dim_color)
        goal_color = world.agents[0].goal_b.color

        # listener pos and vel
        listener_state = [world.agents[1].state.p_pos, world.agents[1].state.p_vel]

        # landmarks pos
        entity_pos = [entity.state.p_pos for entity in world.landmarks]

        return np.around(np.concatenate([goal_color] + listener_state + entity_pos), decimals=world.agents[1].obs_resolution)
