...
class Scenario(BaseScenario):
    def make_world(self, obs_r=2.0, obs_resolution=8, flick_p=0.0, enable_boundary=False):
        world = World()
    ...
    def observation(self, agent, world):
        ...
        for _, i in closest_agents:
            a = world.entities[i]
            if world.cached_dist_mag[i, agent.i] <= agent.obs_range:
                obs.append(world.cached_dist_vect[i, agent.i])
                obs.append(a.state.p_vel)
                obs.append(self.get_agent_encoding(a, world))
            else:
                obs.append(np.zeros_like(world.cached_dist_vect[i, agent.i]))
                obs.append(np.zeros_like(a.state.p_vel))
                obs.append(np.zeros_like(self.get_agent_encoding(a, world)))
        ...
        for _, i in closest_treasures:
            ...
            if world.cached_dist_mag[i, agent.i] <= agent.obs_range:
                ...
                obs.append(np.zeros_like(world.cached_dist_vect[i, agent.i]))
                obs.append(np.zeros_like(np.arange(n_treasure_types) == t.type))
    ...
    def state(self, world):
        ...
        for agent in world.agents:
            if agent.collector:
                agent_encoding.append(np.zeros(n_treasure_types))
                agent_encoding.append(np.arange(n_treasure_types) == agent.holding)
            else:
                agent_encoding.append(np.arange(n_treasure_types) == agent.d_i)
                agent_encoding.append(np.zeros(n_treasure_types))

        # treasure pos
        treasure_pos = [t.state.p_pos for t in world.landmarks]

        # treature type
        treasure_type = [(np.arange(n_treasure_types) == t.type) for t in world.landmarks] 

        return np.concatenate(agent_pos + agent_vel + agent_encoding + treasure_pos + treasure_type)
