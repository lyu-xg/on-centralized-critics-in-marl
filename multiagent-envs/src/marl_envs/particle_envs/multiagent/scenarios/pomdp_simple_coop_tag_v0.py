...
class Scenario(BaseScenario):
    def make_world(self, prey_accel=4.0, prey_max_v=1.3, obs_r=1.0, obs_resolution=8, flick_p=0.0, enable_boundary=False):
        world = World()
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.action_callback = None if i < num_adversaries else self.prey_policy_0
    ...
    def observation(self, agent, world):
        ...
        for entity in world.landmarks:
            if not agent.obs_flick_p:
                ...
                if not entity.boundary and dist <= agent.obs_range:
