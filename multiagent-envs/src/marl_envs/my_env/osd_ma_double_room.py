...
class ObjSearchDelivery(gym.Env):
    ...
    def __init__(self, n_objs=3, terminate_step=150, human_speed_per_step=[[18,18,18,18], [18,18,18,18]], TB_move_speed=0.6, TB_move_noisy=0.0, TB_move_cost=0.0,
                 fetch_pass_obj_tc=4, fetch_look_for_obj_tc=6, fetch_manip_noisy=0.0, fetch_drop_obj_penalty=-10.0, delay_delivery_penalty=0.0, obs_human_wait=False, *args, **kwargs):
...
class ObjSearchDelivery_v5(ObjSearchDelivery):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(ObjSearchDelivery_v5, self).__init__(*args, **kwargs)
    ...
    def createAgents(self):
        ...
        Turtlebot1 = AgentTurtlebot_v4(0, 3.0, 3.5, self.BWPs_T0, self.T_MAs, self.n_objs, speed=self.TB_move_speed, move_noisy=self.TB_move_noisy, 
                                       move_cost=self.TB_move_cost, get_tool_wait=self.fetch_pass_obj_tc+self.fetch_look_for_obj_tc)
        Turtlebot2 = AgentTurtlebot_v4(1, 3.0, 1.5, self.BWPs_T1, self.T_MAs, self.n_objs, speed=self.TB_move_speed, move_noisy=self.TB_move_noisy, 
                                       move_cost=self.TB_move_cost, get_tool_wait=self.fetch_pass_obj_tc+self.fetch_look_for_obj_tc)
    ...
    def step(self, actions, debug=False):
        ...
        for idx, human in enumerate(self.humans):
            ...
            if human.cur_step_time_left <= 0.0 and self.delay_delivery_penalty:
                rewards += self.delay_delivery_penalty
        ...
        return cur_actions, observations, [rewards]*self.n_agent, [len(self.finish_serve_humans) == 2]*self.n_agent, cur_actions_done, {}
...
class ObjSearchDelivery_v6(ObjSearchDelivery):

    """1) Not distinguish Look_for_obj to different robot.
       2) Turtlebot get tool and wait until fetch pass any obj to it or terminate it in 10s.
       3) Turtlebot observes human status but doesn't not have request tool action
       4) Fetch observes which object on the waiting spots instead of how many object on it
       5) Dim of the observation of 4) is equal to n_obj*n_each_obj
       """
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(ObjSearchDelivery_v6, self).__init__(*args, **kwargs)
    ...
    def create_fetch_actions(self):
        ...
        if not self.obs_human_wait:
            # which object is on the waiting spot
            # which turtlebot is beside the table: 2
            self.observation_space_F = spaces.MultiBinary(self.n_objs*self.n_each_obj+2) 
        else:
            self.observation_space_F = spaces.MultiBinary(self.n_objs*self.n_each_obj+2+self.n_human)
    ...
    def _getobs(self, debug=False):
        ...
        if not self.agents[2].cur_action_done:
            observations.append(self.old_observations[2])
            if debug:
                print("Fetchrobot" + " \t which_obj_ready  \t{}".format(self.old_observations[2][0:self.n_objs*self.n_each_obj]))
                print("          " + " \t T#_beside_table  \t{}".format(self.old_observations[2][self.n_objs*self.n_each_obj:self.n_objs*self.n_each_obj+2]))
                if self.obs_human_wait:
                    print("          " + " \t human_wait \t\t{}".format(self.old_observations[2][-self.n_human:]))
                print(" ")
                print("          " + " \t Found_objs  \t{}".format(self.agents[2].found_objs))
            ...
            # get observation about which objects are ready
            F_obs_0 = np.zeros(self.n_objs*self.n_each_obj)
            count_found_obj = [0] * self.n_objs
            for obj_idx in self.agents[2].found_objs:
                count_found_obj[obj_idx] += 1

            for obj_idx in self.agents[2].found_objs:
                obs_id = (count_found_obj[obj_idx]-1)*self.n_objs + obj_idx
                F_obs_0[int(obs_id)] = 1
...
class ObjSearchDelivery_v8(ObjSearchDelivery):
