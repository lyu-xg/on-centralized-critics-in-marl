...
from .core_single_room import AgentTurtlebot_v4, AgentTurtlebot_v7, AgentFetch_v4, AgentHuman, BeliefWayPoint, MacroAction
...
class ObjSearchDelivery(gym.Env):
    ...
    def __init__(self, n_objs=3, terminate_step=150, human_speed_per_step=[[18,18,18,18]], TB_move_speed=0.6, TB_move_noisy=0.0, TB_move_cost=0.0,
                 fetch_pass_obj_tc=4, fetch_look_for_obj_tc=6, fetch_manip_noisy=0.0, fetch_drop_obj_penalty=-10.0, delay_delivery_penalty=0.0, *args, **kwargs):
...
class ObjSearchDelivery_v5(ObjSearchDelivery):
    ...
    """1) Not distinguish Look_for_obj to different robot.
      ...
      *4) Fetch observes which object on the waiting spots instead of how many object on it
       5) delay_delivery_penalty respects to time"""
    ...
    def create_fetch_actions(self):
        ...
        self.observation_space_F = spaces.MultiBinary(self.n_objs+2) # which object is on the waiting spot
    ...
    def createAgents(self):
        ...
        Turtlebot1 = AgentTurtlebot_v4(0, 3.0, 1.5, self.BWPs_T0, self.T_MAs, self.n_objs, speed=self.TB_move_speed, move_noisy=self.TB_move_noisy, 
                                       move_cost=self.TB_move_cost, get_tool_wait=self.fetch_pass_obj_tc+self.fetch_look_for_obj_tc)
        Turtlebot2 = AgentTurtlebot_v4(1, 3.0, 2.5, self.BWPs_T1, self.T_MAs, self.n_objs, speed=self.TB_move_speed, move_noisy=self.TB_move_noisy, 
                                       move_cost=self.TB_move_cost, get_tool_wait=self.fetch_pass_obj_tc+self.fetch_look_for_obj_tc)
    ...
    def step(self, actions, debug=False):
        ...
        #info = list(zip(cur_actions, cur_actions_done))
        ...
        return cur_actions, observations, [rewards]*self.n_agent, [terminate]*self.n_agent, cur_actions_done, {}
    ...
    def _getobs(self, debug=False):
        ...
        if not self.agents[2].cur_action_done:
            ...
            F_obs_0[self.agents[2].found_objs] = 1
...
class ObjSearchDelivery_v7(ObjSearchDelivery):
    ...
    def createAgents(self):
        ...
        #-----------------initialize Two Turtlebot agents
        Turtlebot1 = AgentTurtlebot_v7(0, 3.0, 1.5, self.BWPs_T0, self.T_MAs, self.n_objs, speed=self.TB_move_speed, move_noisy=self.TB_move_noisy, 
        ...
        Turtlebot2 = AgentTurtlebot_v7(1, 3.0, 2.5, self.BWPs_T1, self.T_MAs, self.n_objs, speed=self.TB_move_speed, move_noisy=self.TB_move_noisy, 
                                       move_cost=self.TB_move_cost, get_tool_wait=self.fetch_pass_obj_tc+self.fetch_look_for_obj_tc, delay_delivery_penalty=self.delay_delivery_penalty)
