...
from .box_pushing_MA_core import Agent, astar_Agent, astar_Agent_, astar_Agent_small, Box, BeliefWayPoint, MacroAction
...
class BoxPushing_hard(gym.Env):

    "1) astar low-level controller"
    ...
    def __init__(self, terminate_step=100, random_init=False, *args, **kwargs):
        ...
        MA1 = MacroAction('GT_SB', 0, ma_bwpterm=1)
        MA2 = MacroAction('GT_BB', 1, ma_bwpterm=2)
        MA3 = MacroAction('Push', 2)
        MA4 = MacroAction('T_L', 3, expected_t_cost=1)
        MA5 = MacroAction('T_R', 4, expected_t_cost=1)
        MA6 = MacroAction('Stay', 5, expected_t_cost=1)


        self.MAs = [MA0, MA1, MA2, MA3, MA4, MA5, MA6]
        self.MAs_A0 = [MA0, MA2, MA3, MA4, MA5, MA6]
        self.MAs_A1 = [MA1, MA2, MA3, MA4, MA5, MA6]
    ...
    def createAgents(self):
        if self.random_init:
            ...
            A0 = astar_Agent(0, init_xs[0], init_ys[0], init_ori[0], self.BWPs_A0, self.MAs_A0)
            A1 = astar_Agent(1, init_xs[1], init_ys[1], init_ori[1], self.BWPs_A1, self.MAs_A1)
        else:
            A0 = astar_Agent(0, 1.5, 1.5, 1, self.BWPs_A0, self.MAs_A0)
            A1 = astar_Agent(1, 6.5, 1.5, 3, self.BWPs_A1, self.MAs_A1)
        self.agents = [A0, A1]
