...
class EnvMoveBox(object):
    def __init__(self):
        self.state_space = spaces.Box(
            low=-1, high=1, shape=(15 * 15,), dtype=np.float32
        )
        self.action_space = spaces.Discrete(4)
        self.observation_space = spaces.Box(
            low=-1, high=1, shape=(15*15*3,), dtype=np.float32
        )
    ...
    def step(self, action_list):
        ...
        if self.box_pos == [1, 7]:
            ...
            self.reset()
        return self.get_obs(), float(reward), done or self.i_step >= 100
    ...
    def get_agt1_obs(self):
        o = self.get_global_obs()
        frame = np.zeros((17, 17, 3))
        x_offset = 1  # 0 would be what you wanted
        y_offset = 1
        frame[x_offset:o.shape[0]+x_offset, y_offset:o.shape[1]+y_offset,:] = o

        ci, cj = self.agt1_pos
        return frame[
            ci-2+x_offset:ci+3+x_offset,
            cj-2+y_offset:cj+3+y_offset,:
            ].reshape(-1)
    ...
    def get_obs(self):
        ...
        return [self.get_global_obs().reshape(-1), self.get_global_obs().reshape(-1)]

    def get_state(self):
        res = np.zeros((15, 15))
        res[self.agt1_pos[0], self.agt1_pos[1]] = 1
        res[self.agt2_pos[0], self.agt2_pos[1]] = -1
