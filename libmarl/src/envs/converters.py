...
class Converter(metaclass=ABCMeta):
    ...
    def for_space(space: Space):
        if isinstance(space, spaces.Discrete) or isinstance(space, spaces.MultiBinary):
...
class DiscreteConverter(Converter):
    ...
    def reshape_as_input(self, array: np.ndarray, recurrent: bool):
        return (
            array if recurrent else array.reshape(array.shape[0] * array.shape[1], -1)
        )
    ...
    def policy_out_model(self, in_features: int, out_features=None) -> nn.Module:
        return nn.Linear(in_features, out_features or self.shape[0])
...
class BoxConverter(Converter):
    ...
    def reshape_as_input(self, array: np.ndarray, recurrent: bool):
        return (
            array
            if recurrent
            else array.reshape(array.shape[0] * array.shape[1], *array.shape[2:])
        )
    ...
    def policy_out_model(self, in_features: int, out_features=None) -> nn.Module:
        return NormalDistributionModule(in_features, out_features or self.shape[0])
